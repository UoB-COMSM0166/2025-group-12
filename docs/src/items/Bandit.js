...
import {movableTypes, terrainTypes} from "./ItemTypes.js";
...
export class Bandit extends Enemy {
    ...
    setTarget(playBoard) {
        ...
        if (path.length === 1) {
            InteractionLogic.plantIsAttacked(playBoard, nextCell.plant !== null ? nextCell.plant : nextCell.seed, 1);
            this.hasMoved = true;
            return;
        }
    ...
    graph(playBoard) {
        ...
        for (let cwe of cellsWithEnemy) {
            ...
            if (cwe.enemy && cwe.enemy.movableType === movableTypes.TORNADO) {
                for (let i = 0; i < N - 1; i++) {
