...
export class FloatingWindow {
    constructor(p5, triPos, text, boxParam) {
        ...
        this.x = boxParam.x;
        this.y = boxParam.y;
        this.boxWidth = boxParam.boxWidth;
        this.boxHeight = boxParam.boxHeight;
        this.padding = boxParam.padding;
        this.spacingRatio = boxParam.spacingRatio;
        this.fontSize = this.findBestFontSize(this.text, this.boxWidth - this.padding * 2, this.boxHeight - this.padding * 2);
    }
    ...
    draw() {
        ...
        this.drawFormattedText(this.text, this.x + this.padding, this.y + this.padding + this.p5.textAscent() / 2);
    }
    ...
    findBestFontSize(str, maxWidth, maxHeight) {
        let size = 1;
        let maxSize = 100;

        while (size < maxSize) {
            this.p5.textSize(size);
            let {totalWidth, totalHeight} = this.getTextDimensions(str, size);
